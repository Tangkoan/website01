<div class="w-full max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 space-y-6 relative">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex items-center gap-2 w-full md:w-auto">
            <a href="{{ route('settings.roles') }}" wire:navigate class="px-4 py-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] text-[var(--color-text-main)] font-black rounded-lg hover:brightness-95 transition-all text-sm flex items-center gap-2 shadow-sm">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-[var(--color-background)]/50 border-b border-[var(--color-border-color)]">
                        ...
                        <th class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest w-1/4">{{ __('messages.user_causer') ?? 'User (Causer)' }}</th>
                    ...
                    @forelse($logs as $log)
                        <tr class="hover:bg-[var(--color-background)]/50 transition-all select-none text-sm">
                            ...
                            <td class="p-4 font-bold text-[var(--color-text-main)]">
                                @php
                                    $roleName = $log->subject?->name 
                                                ?? $log->properties['attributes']['name'] 
                                                ?? $log->properties['old']['name'] 
                                                ?? 'Unknown Role';
                                @endphp
                                {{ __('messages.this_role_has_been') ?? 'This role has been' }} 
                                {{ $action }} 
                                <span class="text-[var(--color-primary)]">"{{ $roleName }}"</span>
                            </td>
                        </tr>
                    @empty
            ...
            @forelse($logs as $log)
                <div class="bg-[var(--color-card-bg)] p-4 rounded-xl shadow-sm border border-[var(--color-border-color)] flex flex-col gap-3">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <span class="block text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Causer</span>
