<div class="bg-[var(--color-card-bg)] p-3 rounded-xl border border-[var(--color-border-color)] shadow-sm flex flex-col lg:flex-row items-center justify-between gap-4 mb-4">
    <div class="relative w-full lg:w-96 group">
        ...
        <input type="text" wire:model.live.debounce.300ms="searchTerm" 
            class="w-full h-10 md:h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] focus:border-[var(--color-primary)] text-[var(--color-text-main)] rounded-lg pl-10 pr-4 text-sm font-bold focus:ring-4 focus:ring-[var(--color-primary)]/10 outline-none transition-all" 
            placeholder="{{ __('messages.search_name_or_email') ?? 'Search name or email...' }}">
    </div>

    {{-- ✅ កែប្រែ៖ ប្រើ Alpine.js ដើម្បីបង្ហាញ/លាក់ Animation អោយរលូន និងមិនរំខានដល់ Livewire DOM Diffing --}}
    <div x-data="{ hasSelection: @entangle('selectedUsers').live }" 
         x-show="hasSelection.length > 0" 
         ...
         style="display: none;"
         class="flex flex-col sm:flex-row items-center gap-2 p-1.5 bg-[var(--color-primary)]/10 rounded-lg border border-[var(--color-primary)]/20 w-full lg:w-auto">
