<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    <div>
        <h2 class="text-2xl md:text-3xl font-black text-[var(--color-text-main)] tracking-tight flex items-center gap-3">
            ...
            {{ __('messages.user_management') ?? 'User Management' }}
        </h2>
        <p class="mt-2 text-sm text-[var(--color-text-muted)] transition-colors">{{ __('messages.user_management_desc') }}</p>
    </div>
    
    <div class="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 relative flex-wrap">
        
        {{-- Activity Logs Button (ប្រើ x-auth-link) --}}
        <x-auth-link permission="view-user-logs" 
            href="{{ route('settings.logs', 'user') }}" 
            wire:navigate 
            class="p-2 md:p-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-blue-500 hover:bg-blue-500 hover:text-white transition-all flex items-center gap-2" 
            title="Activity Logs">
        ...
        {{-- Column Selection Dropdown (ទុកធម្មតា ព្រោះអ្នកប្រើគ្រប់រូបគួរតែអាចកំណត់ Column បាន) --}}
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" @click.outside="open = false" type="button" class="p-2 md:p-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-[var(--color-text-main)] hover:brightness-95 transition-all flex items-center gap-2" title="Columns">
                <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"></path></svg>
            </button>

            <div x-show="open" 
                 x-transition.opacity.duration.200ms 
                 class="absolute right-0 md:left-0 mt-2 w-48 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-xl shadow-[0_10px_20px_rgba(0,0,0,0.1)] z-[100] p-2 hidden"
                 :class="{'hidden': !open}">
        ...
        {{-- Trash Button (ប្រើ x-auth-link) --}}
        <x-auth-link permission="view-user-trash" 
            href="{{ route('settings.trash', 'user') }}" 
            wire:navigate 
            class="px-3 py-2 md:px-4 md:py-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center gap-2">
        ...
        {{-- Add New User Button (ប្រើ x-auth-button) --}}
        <x-auth-button permission="create-user" 
            wire:click="openModal" 
            class="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-5 py-2 md:py-2.5 bg-[var(--color-primary)] text-[var(--color-primary-text)] font-black rounded-lg shadow-lg hover:brightness-110 active:scale-95 transition-all text-sm">
            ...
            <span class="hidden md:block">
                {{ __('messages.add_new_user') ?? 'Add New User' }}
            </span>
        </x-auth-button>
    </div>
</div>