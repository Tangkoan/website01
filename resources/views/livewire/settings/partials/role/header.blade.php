<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    ...
    <div class="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 relative">
        ...
        {{-- Activity Logs Button (ប្រើ auth-link) --}}
        <x-auth-link permission="view-role-logs" 
            href="{{ route('settings.logs', 'role') }}" 
            wire:navigate 
        ...
        {{-- Column Selection Dropdown (ទុកធម្មតា ព្រោះអ្នកប្រើប្រាស់គ្រប់រូបគួរតែមានសិទ្ធិរៀបចំ Column តារាងរបស់ពួកគេ) --}}
        <div x-data="{ open: false }" class="relative">
            <button @click="open = !open" @click.outside="open = false" type="button" class="p-2 md:p-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-[var(--color-text-main)] hover:brightness-95 transition-all flex items-center gap-2" title="Columns">
                {{-- រូប SVG របស់ Icon ត្រូវប្ដូរមកដាក់ក្នុង Button វិញ (ក្នុងកូដចាស់របស់អ្នកមានភាពរញ៉េរញ៉ៃបន្តិចត្រង់នេះ) --}}
                <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7m0 10a2 2 0 002 2h2a2 2 0 002-2V7a2 2 0 00-2-2h-2a2 2 0 00-2 2"></path></svg>
            </button>

            {{-- ផ្នែកខាងក្នុង Dropdown ទុកនៅដដែល --}}
            <div x-show="open" 
                 x-transition.opacity.duration.200ms 
                 class="absolute right-0 mt-2 w-48 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-xl shadow-[0_10px_20px_rgba(0,0,0,0.1)] z-[100] p-2 hidden"
                 :class="{'hidden': !open}">
        ...
        {{-- Trash Button (ប្រើ auth-link) --}}
        <x-auth-link permission="view-role-trash" 
            href="{{ route('settings.trash', 'role') }}" 
            wire:navigate 
        ...
        {{-- Add New Button (ប្រើ auth-button ជំនួស @can ចាស់) --}}
        <x-auth-button permission="create-role" 
