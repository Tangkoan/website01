@php
    ...
    $sidebarMenu = \App\Models\Sidebar::where('name', 'story-tag')->first();
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    ...
    <div class="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 relative flex-wrap">
        ...
        <x-auth-link permission="view-story-tag-trash" href="{{ route('settings.trash', 'story-tag') }}" wire:navigate class="px-3 py-2 md:px-4 md:py-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center gap-2">
        ...
        <x-auth-button permission="create-story-tag" wire:click="openModal" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-5 py-2 md:py-2.5 bg-[var(--color-primary)] text-[var(--color-primary-text)] font-black rounded-lg shadow-lg hover:brightness-110 active:scale-95 transition-all text-sm">
