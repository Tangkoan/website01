<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    ...
    <div class="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 relative flex-wrap">
        <x-auth-link permission="view-category-logs" 
            href="{{ route('settings.logs', 'category') }}" 
            wire:navigate 
        ...
        <button wire:click="reloadData" class="p-2 md:p-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-[var(--color-text-main)] hover:brightness-95 transition-all" title="{{ __('messages.reload') ?? 'Reload Data' }}">
        ...
        <x-auth-link permission="view-category-trash" 
            href="{{ route('settings.trash', 'category') }}" 
            wire:navigate 
        ...
        <x-auth-button permission="create-category" wire:click="openModal" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-5 py-2 md:py-2.5 bg-[var(--color-primary)] text-[var(--color-primary-text)] font-black rounded-lg shadow-lg hover:brightness-110 active:scale-95 transition-all text-sm">
