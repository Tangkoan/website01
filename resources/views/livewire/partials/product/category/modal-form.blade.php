    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl bg-black/40 dark:bg-black/60 animate-in fade-in duration-300">
        <div class="bg-[var(--color-card-bg)] w-full max-w-2xl rounded-xl shadow-2xl border border-[var(--color-border-color)] overflow-hidden transition-colors">
            <div class="p-5 border-b border-[var(--color-border-color)] flex justify-between items-center bg-[var(--color-background)]/50 transition-colors">
                <h3 class="text-lg font-bold text-[var(--color-text-main)]">{{ $itemId ? 'Edit Category' : 'Add New Category' }}</h3>
            ...
            <form wire:submit.prevent="saveItem" class="p-6 space-y-6 max-h-[75vh] overflow-y-auto">
                <div class="grid grid-cols-1 gap-4">
                    
                    {{-- Form Fields នឹងត្រូវ Generate ស្វ័យប្រវត្តិតាម DB ក្នុង Phase 2 --}}
                    {{-- សម្រាប់ពេលនេះ យើងដាក់ Field គំរូសិន --}}
                    <div class="space-y-2">
                        <label class="block text-xs font-bold text-[var(--color-text-muted)] uppercase tracking-wider">Name</label>
                        <input type="text" wire:model="name" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none shadow-sm transition-colors" placeholder="Enter category name...">
