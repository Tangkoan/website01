<div class="md:hidden space-y-3">
    ...
    @forelse($items as $item)
        <div wire:key="category-mobile-{{ $item->id }}" 
            x-data 
            @click="if(!$event.target.closest('button') && !$event.target.closest('label') && $event.target.tagName !== 'INPUT') { let cb = $el.querySelector('.row-checkbox'); if(cb && !cb.disabled) cb.click() }" 
            ...
            <div class="flex justify-between items-center">
                <div class="flex items-center gap-4">
                    ...
                    @if(in_array('created_at', $selectedColumns))
                        <span class="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest">{{ $item->created_at ? $item->created_at->format('d M, Y') : 'N/A' }}</span>
                    @endif
                </div>
                
                <div class="flex gap-2">
                    <x-auth-button permission="edit-category" 
                        wire:click.stop="editItem({{ $item->id }})" 
                        class="p-2 rounded-lg transition-all border border-transparent bg-[var(--color-primary)]/10 text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                    </x-auth-button>
                    
                    <x-auth-button permission="delete-category" 
                        wire:click.stop="confirmDelete({{ $item->id }})" 
                        class="p-2 rounded-lg transition-all border border-transparent bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
