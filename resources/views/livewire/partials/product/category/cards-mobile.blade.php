            ...
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0 flex-1 grid grid-cols-2 gap-2">
                                            @if(in_array('name', $selectedColumns)) <div class="flex flex-col"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.name') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate">{{ Str::limit(strip_tags($item->name), 40) }}</span></div> @endif
                    @if(in_array('description', $selectedColumns)) <div class="flex flex-col"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.description') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate">{{ Str::limit(strip_tags($item->description), 40) }}</span></div> @endif
                    @if(in_array('status', $selectedColumns)) <label class="relative inline-flex items-center cursor-pointer"><input type="checkbox" wire:change.stop="toggleStatus({{ $item->id }})" class="sr-only peer" {{ $item->status ? 'checked' : '' }}><div class="w-9 h-5 bg-[var(--color-border-color)] peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-[var(--color-border-color)] after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[var(--color-primary)]"></div></label> @endif
