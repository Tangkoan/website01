        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-[var(--color-background)]/50 border-b border-[var(--color-border-color)]">
                    ...
                    @if(in_array('url', $selectedColumns)) <th wire:click="sortBy('url')" class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest cursor-pointer hover:text-[var(--color-primary)] transition-colors"><div class="flex items-center gap-2">{{ __('messages.url') ?? 'Url' }} @if($sortField === 'url') <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"></path></svg> @endif</div></th> @endif
                    @if(in_array('icon', $selectedColumns)) <th wire:click="sortBy('icon')" class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest cursor-pointer hover:text-[var(--color-primary)] transition-colors"><div class="flex items-center gap-2">{{ __('messages.icon') ?? 'Icon' }} @if($sortField === 'icon') <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"></path></svg> @endif</div></th> @endif
                    @if(in_array('permission', $selectedColumns)) <th wire:click="sortBy('permission')" class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest cursor-pointer hover:text-[var(--color-primary)] transition-colors"><div class="flex items-center gap-2">{{ __('messages.permission') ?? 'Permission' }} @if($sortField === 'permission') <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"></path></svg> @endif</div></th> @endif
                    @if(in_array('order', $selectedColumns)) <th wire:click="sortBy('order')" class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest cursor-pointer hover:text-[var(--color-primary)] transition-colors"><div class="flex items-center gap-2">{{ __('messages.order') ?? 'Order' }} @if($sortField === 'order') <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"></path></svg> @endif</div></th> @endif
                    @if(in_array('is_active', $selectedColumns)) <th wire:click="sortBy('is_active')" class="p-4 text-[10px] font-black text-[var(--color-text-muted)] uppercase tracking-widest cursor-pointer hover:text-[var(--color-primary)] transition-colors"><div class="flex items-center gap-2">{{ __('messages.is_active') ?? 'Is Active' }} @if($sortField === 'is_active') <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"></path></svg> @endif</div></th> @endif
            ...
            <tbody x-data="{ isMouseDown: false, checkStatus: true }" @mousedown="isMouseDown = true" @mouseup.window="isMouseDown = false" class="divide-y divide-[var(--color-border-color)]">
                @forelse($items as $item)
                    <tr wire:key="sidebar-desktop-{{ $item->id }}" @mousedown="if(!$event.target.closest('button') && !$event.target.closest('a') && !$event.target.closest('label') && $event.target.tagName !== 'INPUT' && $event.target.tagName !== 'SVG' && $event.target.tagName !== 'PATH') { let cb = $el.querySelector('.row-checkbox'); if(cb && !cb.disabled) { checkStatus = !cb.checked; cb.checked = checkStatus; cb.dispatchEvent(new Event('change')); } }" @mouseenter="if(isMouseDown && !$event.target.closest('button') && !$event.target.closest('a') && !$event.target.closest('label') && $event.target.tagName !== 'INPUT' && $event.target.tagName !== 'SVG' && $event.target.tagName !== 'PATH') { let cb = $el.querySelector('.row-checkbox'); if(cb && !cb.disabled && cb.checked !== checkStatus) { cb.checked = checkStatus; cb.dispatchEvent(new Event('change')); } }" class="hover:bg-[var(--color-background)]/50 transition-all group cursor-pointer select-none">
                        ...
                        @endif
                        @if(in_array('url', $selectedColumns)) 
                        <td class="p-4 text-sm font-bold text-[var(--color-text-main)] transition-colors">
                            @php $pt = strip_tags($item->url); @endphp
                            {{ Str::limit($pt, 40) ?: '---' }}
                        </td> 
                        @endif
                        @if(in_array('icon', $selectedColumns)) 
                        <td class="p-4 text-sm font-bold text-[var(--color-text-main)] transition-colors">
                            @php $pt = strip_tags($item->icon); @endphp
                            {{ Str::limit($pt, 40) ?: '---' }}
                        </td> 
                        @endif
                        @if(in_array('permission', $selectedColumns)) 
                        <td class="p-4 text-sm font-bold text-[var(--color-text-main)] transition-colors">
                            @php $pt = strip_tags($item->permission); @endphp
                            {{ Str::limit($pt, 40) ?: '---' }}
                        </td> 
                        @endif
                        @if(in_array('order', $selectedColumns)) 
                        <td class="p-4 text-sm font-bold text-[var(--color-text-main)] transition-colors">
                            @php $pt = strip_tags($item->order); @endphp
                            {{ Str::limit($pt, 40) ?: '---' }}
                        </td> 
                        @endif
                        ...
                        <td class="p-4 flex justify-center gap-2 items-center h-full mt-2">
                            <x-auth-button permission="edit-sidebar" wire:click.stop="editItem({{ $item->id }})" title="{{ __('messages.edit') ?? 'Edit' }}" class="p-2 rounded-lg bg-[var(--color-primary)]/10 text-[var(--color-primary)] hover:bg-[var(--color-primary)] hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed relative z-20">
                            ...
                            <x-auth-button permission="delete-sidebar" wire:click.stop="confirmDelete({{ $item->id }})" title="{{ __('messages.delete') ?? 'Delete' }}" class="p-2 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed relative z-20">
