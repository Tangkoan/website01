<div class="md:hidden space-y-3">
    <div class="flex items-center gap-3 px-2 mb-1">
        <input type="checkbox" wire:model.live="selectAll" id="selectAllMobile-story" class="w-4 h-4 rounded border-2 border-[var(--color-text-main)] text-[var(--color-primary)] bg-transparent cursor-pointer">
        <label for="selectAllMobile-story" class="text-sm font-black text-[var(--color-text-main)]">{{ __('messages.select_all') ?? 'Select All' }}</label>
    </div>

    @forelse($items as $item)
        <div wire:key="story-mobile-{{ $item->id }}" x-data @click="if(!$event.target.closest('button') && !$event.target.closest('a') && !$event.target.closest('label') && $event.target.tagName !== 'INPUT' && $event.target.tagName !== 'SVG' && $event.target.tagName !== 'PATH') { let cb = $el.querySelector('.row-checkbox'); if(cb && !cb.disabled) cb.click() }" class="bg-[var(--color-card-bg)] p-4 rounded-xl shadow-sm border border-[var(--color-border-color)] flex flex-col gap-3 cursor-pointer hover:bg-[var(--color-background)]/50 transition-all select-none">
            <div class="flex items-start justify-between gap-3">
                <div class="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div class="min-w-0 flex-1 grid grid-cols-2 gap-2">
                                            @if(in_array('category_id', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.category_id') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ $item->category?->name ?? $item->category_id }}</span></div> @endif
                    @if(in_array('user_id', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.user_id') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ $item->user?->name ?? $item->user_id }}</span></div> @endif
                    @if(in_array('title', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.title') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ strip_tags($item->title) }}</span></div> @endif
                    ...
                    <div class="flex flex-col shrink-0">
                        <span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.thumbnail') }}</span>
                        @php 
                            $f = $item->thumbnail;
                    ...
                    @endif
                    @if(in_array('content', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.content') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ strip_tags($item->content) }}</span></div> @endif
                    ...
                    @if(in_array('views_count', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.views_count') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ strip_tags($item->views_count) }}</span></div> @endif
                    ...
                    @endif
                    @if(in_array('published_at', $selectedColumns)) <div class="flex flex-col min-w-0"><span class="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">{{ __('messages.published_at') }}</span><span class="text-xs font-bold text-[var(--color-text-main)] truncate block">{{ strip_tags($item->published_at) }}</span></div> @endif
            ...
            <div class="flex justify-between items-center mt-2">
                <div class="flex gap-2">
                    ...
                    <x-auth-button permission="delete-story" wire:click.stop="confirmDelete({{ $item->id }})" class="p-2 rounded-lg transition-all border border-transparent bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed relative z-20">
