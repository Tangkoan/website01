            ...
            <div class="w-full md:w-1/3 bg-[var(--color-background)] border-r border-[var(--color-border-color)] flex flex-col max-h-[35vh] md:max-h-full">
                ...
                <div class="p-3 overflow-y-auto flex-1 space-y-2 no-scrollbar">
                    @foreach($selectedItemsQueue as $index => $id)
                        @php $itemName = \App\Models\Story::find($id)?->name ?? 'Unknown'; @endphp
                ...
                <form wire:submit.prevent="saveAndNextBulkItem" class="flex-1 overflow-y-auto p-5 md:p-6 flex flex-col">
                    <div class="space-y-4 flex-1">
                        ...
                        <div class="w-full md:w-3/4">
                            <select wire:model="bulkItem_category_id" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none cursor-pointer">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 border-b border-[var(--color-border-color)] pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                        ...
                        <div class="w-full md:w-3/4">
                            <select wire:model="bulkItem_user_id" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none cursor-pointer">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 border-b border-[var(--color-border-color)] pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                        ...
                        <div class="w-full md:w-3/4">
                            <input type="text" wire:model="bulkItem_title" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none shadow-sm transition-colors" placeholder="Enter Title...">
                            @error('bulkItem_title') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 border-b border-[var(--color-border-color)] pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                    ...
                    <div class="flex flex-col md:flex-row md:items-start gap-2 md:gap-4 p-4 bg-[var(--color-background)]/30 rounded-xl border border-[var(--color-border-color)] border-dashed mb-4 last:mb-0">
                        ...
                        <div class="w-full md:w-3/4">
                            <input type="file" wire:model="bulkItem_thumbnail" id="f-bulkItem_thumbnail"  accept="image/*" class="hidden">
                            <label for="f-bulkItem_thumbnail" class="px-4 py-2 bg-[var(--color-primary)] text-white text-xs font-bold rounded-lg cursor-pointer inline-block">Upload Thumbnail</label>
                            @if ($bulkItem_thumbnail) 
                                <div class="mt-3 flex flex-wrap gap-2"> 
                                    @if(is_array($bulkItem_thumbnail) || is_iterable($bulkItem_thumbnail)) 
                                        @foreach($bulkItem_thumbnail as $i => $f) 
                                            <img src="{{ is_string($f) ? (str_starts_with($f, 'http') ? $f : asset('storage/'.$f)) : $f->temporaryUrl() }}" class="h-20 w-20 rounded-lg object-cover"> 
                                        @endforeach 
                                    @else 
                                        <img src="{{ is_string($bulkItem_thumbnail) ? (str_starts_with($bulkItem_thumbnail, 'http') ? $bulkItem_thumbnail : asset('storage/'.$bulkItem_thumbnail)) : $bulkItem_thumbnail->temporaryUrl() }}" class="h-20 w-20 rounded-lg object-cover"> 
                                    @endif 
                                </div> 
                            @endif
                        </div>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 border-b border-[var(--color-border-color)] pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                        ...
                        <div class="w-full md:w-3/4">
                            <input type="text" wire:model="bulkItem_views_count" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none shadow-sm transition-colors" placeholder="Enter Views Count...">
                            @error('bulkItem_views_count') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 border-b border-[var(--color-border-color)] pb-4 mb-4 last:border-0 last:pb-0 last:mb-0">
                        ...
                        <div class="w-full md:w-3/4">
                            <input type="text" wire:model="bulkItem_published_at" class="w-full h-11 bg-[var(--color-background)] border border-[var(--color-border-color)] text-[var(--color-text-main)] rounded-lg px-4 text-sm focus:ring-2 focus:ring-[var(--color-primary)] outline-none shadow-sm transition-colors" placeholder="Enter Published At...">
                            @error('bulkItem_published_at') <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> @enderror
                        </div>
                    </div>
