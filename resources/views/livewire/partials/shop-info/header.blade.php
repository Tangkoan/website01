@php
    ...
    $sidebarMenu = \App\Models\Sidebar::where('name', 'shop-info')->first();
...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
    <div>
        <h2 class="text-2xl md:text-3xl font-black text-[var(--color-text-main)] tracking-tight flex items-center gap-3">
            ...
            {{ __('messages.shop-info_management') ?? 'ShopInfos Management' }}
        </h2>
    </div>
    
    <div class="flex items-center gap-2 w-full md:w-auto mt-2 md:mt-0 relative flex-wrap">
        <x-auth-link permission="view-shop-info-logs" href="{{ route('settings.logs', 'shop-info') }}" wire:navigate class="p-2 md:p-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-blue-500 hover:bg-blue-500 hover:text-white transition-all flex items-center gap-2" title="{{ __('messages.logs') ?? 'Logs' }}">
        ...
        <x-auth-link permission="view-shop-info-trash" href="{{ route('settings.trash', 'shop-info') }}" wire:navigate class="px-3 py-2 md:px-4 md:py-2.5 bg-[var(--color-card-bg)] border border-[var(--color-border-color)] rounded-lg shadow-sm text-red-500 hover:bg-red-500 hover:text-white transition-all flex items-center gap-2">
        ...
        <x-auth-button permission="create-shop-info" wire:click="openModal" class="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-5 py-2 md:py-2.5 bg-[var(--color-primary)] text-[var(--color-primary-text)] font-black rounded-lg shadow-lg hover:brightness-110 active:scale-95 transition-all text-sm">
