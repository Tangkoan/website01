...
class SettingManagement extends Component
{
    ...
    public $bulkItem_key;
    public $bulkItem_value;
    
    public $isModalOpen = false, $searchTerm = '', $perPage = 10;
    public $sortField = 'id', $sortDirection = 'desc';
    
    public $availableColumns = ['key' => 'Key', 'value' => 'Value'];
    public $selectedColumns = ['key', 'value']; 
    ...
    protected function service() { return app(SettingService::class); }
    ...
    public function bulkEdit() {
        abort_if(Gate::denies('edit-setting'), 403);
    ...
    private function loadBulkItemData($index) {
        ...
        $item = Setting::find($this->selectedItemsQueue[$index]);
        if ($item) {
            $this->bulkItemId = $item->id;
            $this->bulkItem_key = $item->key;
            $this->bulkItem_value = $item->value;
        }
    }
    ...
    public function saveAndNextBulkItem() {
        abort_if(Gate::denies('edit-setting'), 403);
        $this->validate([
            'bulkItem_key' => 'required|string|max:255',
            'bulkItem_value' => 'nullable|string',
        ]);
        $this->service()->saveItem([
            'key' => $this->bulkItem_key,
            'value' => $this->bulkItem_value,
        ], $this->bulkItemId);
    ...
    public function closeBulkEdit() {
        ...
        $this->reset(['selectedItemsQueue', 'currentBulkIndex', 'bulkItemId', 'selectedItems', 'selectAll', 'bulkItem_key', 'bulkItem_value']);
        $this->resetErrorBag();
    }
    ...
    public function openModal() {
        abort_if(Gate::denies('create-setting'), 403);
        $this->reset(['itemId']);
    ...
    public function saveItem() {
        if ($this->itemId) abort_if(\Illuminate\Support\Facades\Gate::denies('edit-setting'), 403);
        else abort_if(\Illuminate\Support\Facades\Gate::denies('create-setting'), 403);
    ...
    public function confirmDelete($id = null) {
        abort_if(Gate::denies('delete-setting'), 403);
        $this->deleteId = $id;
    ...
    public function render() {
        ...
        ])->title(__('messages.setting_management') ?? 'Settings Management');
    }
}